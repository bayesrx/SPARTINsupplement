...
description: |  
  This is "living documentation" of the SPARTYN pipeline
link-citations: yes
...
expected value of an indicator random variable with respect to the 
joint distribution of $\gamma_{ik,12}$ and $\gamma_{ik,12,0}$ (i.e. the 
value of the interaction parameter under the null distribution), this 
naturally yields an interpretation of $r_{ik}$ as a probability. 
Specifically, it can be thought of as measuring the posterior probability 
that the observed value of $\gamma_{ik,12}$ (with posterior distribution $f_{ik|\mathbf{p}}(\cdot)$) is larger than the value of $\gamma_{ik,12,0}$
(with posterior distribution $f_{ik,0}(\cdot)$). We thus refer to $r_{ik}$ 
as the Infiltration Probability for that particular tile, which can be
aggregated across tiles to yield a measure of infiltration on a particular
biopsy.
...
than what one would expect *by chance alone.* While it would perhaps be
