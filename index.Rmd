...
samples as well as subsections of tissue samples (@Saltz18; @Amgad19; @Lu20;
@Negahbani21; @Bian21). This work, often broadly 
...
cancer biopsies to the classification of cells in tumor biopsies (@Komura18). This work has important implications for 
...
be valuable. @Pages10 proposed immune reaction as the seventh hallmark of cancer, and laid out different associations between various types of immune cells and outcomes of interest. Of the different types of immune cells, the most well-studied are lymphocytes. 
...
in colorectal cancer (@Idos20), Breast cancer 
(@Dieci18; @Denkert15), and melanoma 
(@Fu19). This has led to increased interest in Computational 
...
with survival in certain types of  cancer. @Lu20 also 
...
various organisms (@Hogsar99; @King12). More 
recently, methods from this paradigm have been successfully 
applied within the domain of Biostatistics (@Kang11; 
@Ray15). Given the relatively simple and granular nature 
...
@LiMIM19 while still allowing for the assessment of full 
images in the style of @Saltz18. We accomplish this by 
...
associated with biopsy-level gene expression among genes related to immune cells. Additionally, we found that CTIP was significantly associated with various outcomes as assessed by @Akbani15, including transcriptomic class and pathologist assessment.
...
(@Franklin17; @Achkar17).  It is possible that 
...
amongst the various cancer  types (@Berger12).  The 
...
Except in the trivial case where $\gamma_{12} = 1$, the normalizing constant of this distribution is computationally intractable. This motivates the usage of the pseudolikelihood as outlined in @BT00. For the density $f(\cdot)$ of the simplified Hierarchical Strauss model outlined above, define the conditional intensity function at a point $u$ given $\theta = \{\beta_1, \beta_2, \gamma_{12}\}$ and points $\bm{x}$ observed in window $A$ by 
...
Using this approximation of the pseudolikelihood function in place of the more standard likelihood fnction, Bayesian analysis can proceed in the style of @King12 by simply assigning priors to the parameters of interest and using techniques to sample from non-closed form posterior likelihoods. We assigned non-informative normal priors with mean 0 and variance $10^6$ to $\log(\gamma_{12})$, $\log(\beta_1)$, and $\log(\beta_2)$. In all analysis presented below the quadrature and weights used to estimate the integral in the pseudolikelihood function was generated by the spatstat package (@BT05). Samples from the posterior were taken using JAGS via the R2jags package 
(@Su15).
...
\paragraph{\textbf{Association with expression of significantly mutated genes}} In addition to associations with survival, we investigated the 
...
(@LiMIM19). As with the CTIP model, MCON was standardized
