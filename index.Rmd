...
description: |  
  Supplementary material related to  the SPARTYN pipeline
...
samples as well as subsections of tissue samples (\citealp{Saltz18}; \citealp{Amgad19}; \citealp{Lu20};
\citealp{Negahbani21}; \citealp{Bian21}). This work, often broadly 
...
cancer biopsies to the classification of cells in tumor biopsies (\citealp{Komura18}). This work has important implications for 
...
in colorectal cancer (\citealp{Idos20}), Breast cancer 
(\citealp{Dieci18}; \citealp{Denkert15}), and melanoma 
(\citealp{Fu19}). This has led to increased interest in Computational 
...
with survival in certain types of  cancer. \citealp{Lu20} also 
...
various organisms (\citealp{Hogsar99}; \citealp{King12}). More 
recently, methods from this paradigm have been successfully 
applied within the domain of Biostatistics (\citealp{Kang11}; 
\citealp{Ray15}). Given the relatively simple and granular nature 
...
<!-- \begin{figure}[!ht] -->
<!-- % \begin{minipage}{17cm}{c} -->
<!-- \centering -->
<!-- \includegraphics[width=16cm]{Figures/pipeline_and_cartoon.png} -->
<!-- \caption{An illustration of spatial interaction between cells of different types and an overview of the pre-processing pipeline. 1a) depicts different strengths of spatial interaction. (A) depicts low/no interaction; (B) depicts moderate interaction; (C) depicts strong interaction. 1b) shows an overview of the SPARTIN pipeline, from pre-processing to biopsy level assessment. Raw biopsy images (A) are pre-processed into cell types (tumor cells or immune cells, colored black and red respectively) and locations. The points are intensity thresholded, removing excess white space from the image and producing a more closely fitted window (B). Next, the biopsy is tessellated into non-overlapping sub-regions containing comparable numbers of tumor cells (C). Models are fit on each tile (D), and tile-specific Cell Type Interaction Probability is calculated from the resulting posterior distributions (E), allowing us to capture heterogeneity in infiltration across the biopsy in a manner that can be aggregated (F) and appropriately summarized (G).}\label{fig:01} -->
<!-- % \end{minipage} -->
<!-- \end{figure} -->
...
\citealp{LiMIM19} while still allowing for the assessment of full 
images in the style of \citealp{Saltz18}. We accomplish this by 
...
associated with biopsy-level gene expression among genes related to immune cells. Additionally, we found that CTIP was significantly associated with various outcomes as assessed by \citealp{Akbani15}, including transcriptomic class and pathologist assessment.
...
(\citealp{Franklin17}; \citealp{Achkar17}).  It is possible that 
...
amongst the various cancer  types (\citealp{Berger12}).  The 
...
\no{My overarching question for this section is how to incorporate the data structure. What I'm doing now is outlining it near the beginning, and bringing it up where relevant. However, I feel like it may be preferable to spend the first subsections only discussing point processes and CTIP at the level of an individual tile, i.e. a well defined window with points inside of it. Once that's established, I could conclude by discussing the data structure. The main reason for doing this is that while the data structure informs the intensity thresholding and partitioning of the biopsies, it doesn't inform the actual formulation of the model or the details of the model fitting. It does, however, complicate the notation, and forces me to go in and out of a purely statistical explanation of the model more than I might otherwise. Thus, I wonder if it might make the most sense to simply leave it for last. I'm also wondering if it would be less confusing to only present the "canonical" parameterizations (i.e. using the log parameters)}
...
Let $B_1...B_n$ be a set of independent regions with $B_i \in \mathbb{R}^2 \times \mathbb{R}^2$, such that each $B_i$ has been partitioned into $n_i$ non-overlapping sub-regions $b_{i1}..b_{in_i}$ each with well-defined boundaries. Denote the set of points observed within sub-region $j$ of region $i$ by $\bm{x}_{ij}$, and the marks of those points as $\bm{m}_{ij}$ where each mark $m_{ijk} \in \{K_1,...,K_\ell\}$. Within each sub-region, this data can be naturally viewed as the realization of a marked point process and modeled as a Hierarchical Multitype Strauss Process.
...
(\citealp{Strauss75}) \no{While this paper introduced Strauss models, the original model actually couldn't model positive interaction, which was pointed out a year later in a different paper (hence our usage of the hierarchical variant). Is this worth mentioning?}. As for the specific decision to use 
...
\begin{equation}
\label{eqn:density}
f(\bm{p}_1, \bm{p}_2) \propto 
\beta_1^{n_1} \beta_2^{n_2}
\gamma_{11}^{S_{R_{11}}(\bm{p}_1)}
\gamma_{22}^{S_{R_{2}}(\bm{p}_2)}
\gamma_{12}^{S_{R_{12}}(\bm{p}_1, \bm{p}_2)}
\tag{1}  
\end{equation}


where $\bm{p}_t$ is a vector of points of type $t$, $n_t$ is the number of points of type $t$, $\beta_t$ is the first order intensity of points of  type $t$, $S_{R_{tl}}(\cdot)$ counts the number of pairs of points of  types $t$ and $l$ within $R_{tl}$ of one another where $R_{tl}$ is  selected a priori based on subject specific knowledge, and $\gamma_{tl}$ captures the tendency of points of type $l$ to be near points of type $t$.
...
Except in the trivial case where $\gamma_{12} = 1$, the normalizing constant of this distribution is computationally intractable. This motivates the usage of the pseudolikelihood as outlined in \citealp{BT00}. For the density $f(\cdot)$ of the simplified Hierarchical Strauss model outlined above, define the conditional intensity function at a point $u$ given $\theta = \{\beta_1, \beta_2, \gamma_{12}\}$ and points $\bm{x}$ observed in window $A$ by 

\begin{equation}
\label{eqn:CL}
\lambda(u|\theta, \bm{x}) = 
\begin{cases}
\frac{f(\bm{x} \cup \{u\})}{f(\bm{x})} & u \not\in \bm{x} \\
\frac{f(\bm{x})}{f(\bm{x} - \{u\})} & u \in \bm{x}
\end{cases}
\tag{2}
\end{equation}
...
\label{eqn:PL}
PL(\theta | \bm{x}) = 
\prod_{x_i \in \bm{x}} \lambda(x_i | \theta, \bm{x}) \exp(-\int_A \lambda(u | \theta, \bm{x}) du) \tag{3}
\end{equation}

This provides a computationally tractable alternative to the likelihood function that can be used in inference. Specifically, the integral in the pseudolikelihood function can be easily approximated by summing over a weighted quadrature on $A$. Thus, given a quadrature $\bm{u}$ on $A$ with corresponding weights $\bm{w}$, Equation (\ref{eqn:PL}) can be approximated by

\begin{equation}
\label{eqn:PL_approx}
PL(\theta | \bm{x}) \approx
\prod_{x_i \in \bm{x}} \lambda(x_i | \theta, \bm{x}) \exp(-\sum_{u_j \in \bm{u}} \lambda(u_j | \theta, \bm{x}) w_j) \tag{4}
\end{equation}
...
Using this approximation of the pseudolikelihood function in place of the more standard likelihood fnction, Bayesian analysis can proceed in the style of \citealp{King12} by simply assigning priors to the parameters of interest and using techniques to sample from non-closed form posterior likelihoods. We assigned non-informative normal priors with mean 0 and variance $10^6$ to $\log(\gamma_{12})$, $\log(\beta_1)$, and $\log(\beta_2)$. In all analysis presented below the quadrature and weights used to estimate the integral in the pseudolikelihood function was generated by the spatstat package (\citealp{BT05}). Samples from the posterior were taken using JAGS via the R2jags package 
(\citealp{Su15}).
...
\no{Should this subsection come last? If so, is there any point in mentioning the data structure ("regions" $\rightarrow$ "sub-regions" $\rightarrow$ points) before this subsection?}
...
## Cell Type Interaction Probability
\no{Right now I am putting this section last, because the notation depends on the notion of the "tiles"/partition. If we re-organize this section so that the data structure explanation goes last, this could come right after the section on the pseudolikelihood function, which I think would flow best.}
...
\paragraph{\textbf{Definition}} Let $f_{i_{(k)}}(\gamma)$ be the true posterior distribution of $\log(\gamma_{i_{(k)}})$, and $f_{i_{(k)},0}(\gamma_0)$ be the distribution of the log-interaction parameter absent interaction conditional upon the location of the tumor cells in tile $k$ of biopsy $i$. Further, assume independence between the true distribution and the null distribution. Then we define the CTIP for tile $k$ of biopsy $i$ by:
...
\paragraph{\textbf{Estimating the empirical null distribution}} The problem of estimating CTIP hinges on being able to estimate and sample from the null distribution of $\log(\gamma_{i_{(k)}})$, since estimation of the true posterior can proceed as described in section 2.3. 
...
\no{I could get even more rigorous with the notation and "algorithm" here, but this section is getting quite long as is- let me know if you think that would be a good idea.}
...
simulated within $30$ $\mu m$ of a\ randomly selected tumor cell.

<!-- \begin{figure}[!ht] % Simulation Figure -->
<!-- \centering -->
<!-- \includegraphics[width=14cm]{Figures/Simulation_Figure.png} -->
<!-- \caption{Top: examples of positive and negative simulations for different values of p, all from simulation setting 1. Bottom left: box plots of CTIP across all simulated compositions for different values of p on logit scale. Bottom right: ROC curves and associated AUC for classification (positive interaction vs. non-positive interaction) for different simulation settings. Simulation setting 1 consisted of 20 tumor cells and 50 immune cells; simulation setting 2 consisted of 100 tumor cells and 100 immune cells; simulation setting 3 consisted of 50 tumor cells and 20 immune cells; and simulation setting 4 consisted of 50 tumor cells and 50 immune cells.} -->
<!-- \label{fig:03} -->
<!-- \end{figure} -->

<!-- \begin{figure}[!ht] -->
<!-- {\centering -->
<!-- \includegraphics[width=16cm]{Figures/data_violin_forest.png} \\} -->
<!-- \caption{(A) selected examples of actual biopsies, both raw data and with tessellations and CTIP by tile. The leftmost biopsy pictured has among the lowest mean estimated CTIP in the sample (0.4), while the rightmost has among the highest (0.94). The middle biopsy has average mean CTIP (0.7), but also exhibits more spatial variation than the left or right biopsy. Bottom: distribution of mean CTIP by biopsy for all biopsies investigated. This illustrates the between-biopsy variation in overall levels of infiltration. (B) hazard ratios from Cox Proportional Hazards models.  -->
<!-- Points at 1 indicate reference category. Age, IP, and MCON -->
<!-- were all standardized by subtracting the mean value and  -->
<!-- dividing by the standard deviation. Normalized logit-IP  -->
<!-- was significantly associated with increased risk of death -->
<!-- $(p = 0.019)$, while normalized logit-MCON was  -->
<!-- non-significantly associated with an increased risk of death -->
<!-- $(p = 0.057)$.(C): Average Logit CTIP by transcriptomic class according to Akbani et al. 2015.} -->
<!-- \label{fig:04} -->
<!-- \end{figure} -->

<!-- \begin{figure}[!ht] % Genomic Table -->
<!-- {\centering  -->
<!-- \includegraphics[width=4.5cm]{Figures/Genomic Table.png} \\} -->
<!-- Table 1: P-values of univariate regression coefficients -->
<!-- for 10 most significant genes. An asterisk indicates -->
<!-- significance under a Benjamini-Hochberg correction. -->
<!-- \end{figure} -->

Otherwise, $l_i$ was simulated from a Poisson process. 
...
While we are broadly grouping immune cells together for the purposes of this analysis, it is important to note that within the microevironment of melanoma there is a great deal of heterogeneity in the types of immune cells present. Tumor Infiltrating Lymphocytes (TILs) receive a disproportionate amount of attention in cancer literature due to the well-established prognostic value of the presence. However, in the context of melanoma this focus can obscure the tremendous amount of variability in the types of immune cells present in the microenvironment. Even referring to TILs as a monolith obfuscates the different cell types captured under this umbrella, many of which serve different purposes (\citealp{Antohe19}, \citealp{Mukherji13}, \citealp{Weiss16}). The richness of the immunological aspect of the melanoma microenvironment makes it a logical target of analysis via the SPARTIN pipeline. And while advances in modern technologies may allow for a more granular understanding of the exact spatial role of these various cell types, a broad first-order spatial analysis allows for the detection of arbitrary spatial dynamics that can be further investigated as technical constraints ease. \no{I could say more here along the lines of clarifying that right now the goal is hypothesis generation.}

Another reason to consider melanoma is the uniquely rich literature regarding the potential applications of immunotherapy. There are many factors that have contributed to this, not the least of which is the relative accessibility of melanoma compared to other types of cancer. Regardless, the net result is a far more mature understanding of immunotherapy in the context of melanoma than other types of cancer (\citealp{Oble09}, \citealp{Nguyen10}, \citealp{Pitcovski17}). Thus, possible implications of the spatial immune organization in melanoma for immunotherapy may be more readily interpreted within the context of melanoma than other cancer types, particularly as the ability to distinguish between different types of immune cells improves.

In order to compute CTIP, 
images were processed and models were fit in using the methods
described in section 2. The same settings were used for all
biopsies across the pipeline. The mean number of tumor and immune cells in each biopsy was 112,258 (standard deviation 67,275). For tumor cells and immune cells, the mean numbers were 29,570 and 82,688 respectively (standard deviations 18,504 and 53,890, respectively). \no{This feels like a bad way to summarize- should I give first, second and third quartiles instead?} For a sample of results, see Figure 3. The color of each tile indicates the value of CTIP estimated for that tile. More varied colors across a given biopsy are indicative of more spatial variation in CTIP, and thus infiltration. CTIP was summarized at the biopsy level by taking the empirical mean across all tiles for a given biopsy. Across biopsies, the median biopsy level CTIP was 0.69, and the interquartile range was 0.19. These results are consistent with the conventional wisdom that melanoma is a generally more immunogenic cancer. 
...
our sample using TCGA Assembler (\citealp{Zhu14}). Additionally, 
...
(\citealp{Uno11}) of the Cox model relative to the model that 
...
(\citealp{LiMIM19}). As with the CTIP model, MCON was standardized
